        ...
        <div className="product-detail-card" style={{ gridTemplateColumns: '1.2fr 0.8fr', alignItems: 'start' }}>
          ...
          <div className="cart-items-section">
            ...
            <div className="orders-table" style={{ padding: '0', background: 'transparent', boxShadow: 'none', border: 'none' }}>
              {cartItems.map((item) => (
                <div key={item.product._id} className="orders-row" style={{ gridTemplateColumns: '80px 2fr 1fr auto auto', gap: '16px', background: 'var(--surface-2)' }}>
                  <img 
                  ...
                  />
                  <div>
                    <h4 style={{ margin: '0 0 6px 0' }}>{item.product.name}</h4>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-3)' }}>MOQ: {item.product.minQuantity || 1}</p>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-2)' }}>Tk {item.product.price.toFixed(2)}</p>
                  </div>
